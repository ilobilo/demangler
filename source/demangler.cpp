...
namespace absl
{
    namespace debugging_internal
    {
        ...
        static void Append(State *state, const char *const str, const size_t length)
        {
            ...
            {
                if (state->parse_state.out_cur_idx + 1 < state->out_end_idx)
                { // +1 for '\0'
        ...
        {
            if (state->parse_state.nest_level >= 1 &&
                state->parse_state.append && state->parse_state.out_cur_idx >= 2)
            {
        ...
        static void ParseModuleDiscriminator(State *state);
        ...
        // <module-subname> ::= W <source-name>      # module name component
        //                  ::= W P <source-name>    # module partition component
        static void ParseModuleDiscriminator(State *state)
        {
            while (RemainingInput(state)[0] == 'W')
            {
                ParseState copy = state->parse_state;
                bool prev_append = state->parse_state.append;
                DisableAppend(state);

                if (!ParseOneCharToken(state, 'W'))
                {
                    RestoreAppend(state, prev_append);
                    break;
                }
                // Optional 'P' marks a module partition boundary.
                ParseOneCharToken(state, 'P');

                if (!ParseSourceName(state))
                {
                    // Not a valid module subname; back out entirely.
                    state->parse_state = copy;
                    RestoreAppend(state, prev_append);
                    break;
                }
        ...
        static bool ParseUnnamedTypeName(State *state)
        {
            ...
            if (ParseTwoCharToken(state, "Ut") &&
                Optional(ParseNumber(state, &which)) &&
        ...
        static bool ParseOperatorName(State *state, int *arity)
        {
            ...
            if (ParseTwoCharToken(state, "cv") &&
                ...
                EnterNestedName(state) &&
                ParseConversionOperatorType(state) &&
                LeaveNestedName(state, copy.nest_level))
            {
                if (arity != nullptr)
                    *arity = 1;
        ...
        static bool ParseSpecialName(State *state)
        {
            ...
            if (ParseTwoCharToken(state, "Tc") &&
                ParseCallOffset(state) &&
                ParseCallOffset(state) &&
                ParseEncoding(state))
                return true;
            state->parse_state = copy;
        ...
        static bool ParseCtorDtorName(State *state)
        {
            ...
            if (ParseOneCharToken(state, 'C'))
            {
                ...
                else if (ParseOneCharToken(state, 'I') &&
                    ParseCharClass(state, "12") &&
        ...
        static bool ParseType(State *state)
        {
            ...
            if (ParseTwoCharToken(state, "Dk") &&
                ParseTypeConstraint(state))
                return true;
            state->parse_state = copy;
        ...
        static bool ParseArrayType(State *state)
        {
            ...
            if (ParseOneCharToken(state, 'A') &&
                Optional(ParseExpression(state)) &&
                ParseOneCharToken(state, '_') &&
                ParseType(state))
                return true;
            state->parse_state = copy;
        ...
        static bool ParseBaseUnresolvedName(State *state)
        {
            ...
            ParseState copy = state->parse_state;
            if (ParseTwoCharToken(state, "on") &&
                ParseOperatorName(state, nullptr) &&
                Optional(ParseTemplateArgs(state)))
                return true;
            state->parse_state = copy;
        ...
        static bool ParseFunctionParam(State *state)
        {
            ...
            if (ParseTwoCharToken(state, "fp") &&
                Optional(ParseCVQualifiers(state)) &&
                Optional(ParseNumber(state, nullptr)) &&
                ParseOneCharToken(state, '_'))
                return true;
            state->parse_state = copy;
        ...
        static bool ParseExpression(State *state)
        {
            ...
            if (ParseTwoCharToken(state, "cl") &&
                OneOrMore(ParseExpression, state) &&
                ParseOneCharToken(state, 'E'))
                return true;
            state->parse_state = copy;
            ...
            if (ParseTwoCharToken(state, "cp") &&
                ParseSimpleId(state) &&
                ZeroOrMore(ParseExpression, state) &&
                ParseOneCharToken(state, 'E'))
                return true;
            state->parse_state = copy;
            ...
            if (ParseCharClass(state, "dscr") &&
                ParseOneCharToken(state, 'c') &&
                ParseType(state) &&
                ParseExpression(state))
                return true;
            state->parse_state = copy;
            ...
            else
            {
                ...
                if (ParseOperatorName(state, &arity) && arity > 0 && // 0 arity => disabled.
                    ...
                    (arity < 2 || ParseExpression(state)) && (arity < 1 || ParseExpression(state)))
                    return true;
            }
            state->parse_state = copy;
            ...
            if ((ParseTwoCharToken(state, "dt") ||
                ParseTwoCharToken(state, "pt")) &&
                ParseExpression(state) &&
            ...
            if (ParseTwoCharToken(state, "rq") &&
                OneOrMore(ParseRequirement, state) &&
                ParseOneCharToken(state, 'E'))
                return true;
            state->parse_state = copy;
        ...
        static bool ParseLocalNameSuffix(State *state)
        {
            ...
            if (MaybeAppend(state, "::") && ParseName(state) && Optional(ParseDiscriminator(state)))
                return true;
            state->parse_state = copy;
            if (state->parse_state.append && state->parse_state.out_cur_idx < state->out_end_idx)
                state->out[state->parse_state.out_cur_idx] = '\0';
        ...
        bool Demangle(const char *mangled, char *out, size_t out_size)
        {
            ...
            return ParseTopLevelMangledName(&state) &&
                !Overflowed(&state) && state.parse_state.out_cur_idx > 0;
        }
    } // namespace debugging_internal
} // namespace absl
